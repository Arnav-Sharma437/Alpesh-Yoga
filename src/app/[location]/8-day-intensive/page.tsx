...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `8-Day Intensive Hatha Alignment Course in ${config.displayName} | Alpesh Yoga`,
    description: `Deepen your alignment at our 8-Day Intensive Hatha Alignment Course in ${config.displayName}. Systematically correct your posture using ropes, blocks, and belts.`,
    keywords: [
      `Hatha alignment intensive ${config.slug}`,
      `yoga alignment course ${config.slug}`,
      `8 day yoga course ${config.slug}`,
      `Alpesh Yoga alignment intensive`
    ],
  };
}

export default async function IntensiveCourse({ params }: PageProps) {
  const { location } = await params;
  const config = locationsConfig[location];
  if (!config) {
    notFound();
  ...
  const RELATED_PROGRAMS = [
    {
      title: "Daily Classes",
      desc: "Join our drop-in Hatha alignment sessions available every morning and evening in our studio shala.",
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              ...
              <div className="space-y-4">
                ...
                <ul className="space-y-3 font-sans text-sm text-forest-600/80">
                  ...
                  <li className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-sage-500 shrink-0" />
                    <span>Wall-rope traction stations access (if available on-site)</span>
                  </li>
                  <li className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-sage-500 shrink-0" />
            ...
            <div className="bg-white border border-sage-100/60 rounded-3xl p-8 shadow-sm flex flex-col md:flex-row gap-8 items-center justify-between">
              ...
              <div className="space-y-2">
                ...
                <p className="font-sans text-xs text-forest-600/60 leading-normal max-w-md">
                  Please check our dynamic intake start schedules for {config.displayName}:
                </p>
              </div>
              ...
              <div className="w-full md:w-60 font-mono text-xs text-forest-700 divide-y divide-sage-50 bg-cream-50 p-4 rounded-2xl border border-sage-50">
                {config.intensiveDates.map((date, idx) => (
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="bg-white border border-sage-200/80 rounded-3xl p-8 md:p-12 shadow-md max-w-md mx-auto text-center space-y-6">
              ...
              <p className="font-sans text-xs text-sage-600 leading-normal">
                Deposit amount: **{config.intensivePricing.deposit}** to secure your booking.
              </p>
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="bg-gradient-to-br from-terracotta-700 to-terracotta-600 rounded-3xl p-8 md:p-12 text-center text-cream-50 shadow-xl border border-terracotta-800 space-y-6">
              ...
              <p className="font-sans text-sm text-cream-100/90 leading-relaxed font-light max-w-xl mx-auto">
                Due to our focus on physical adjustments and individual alignment blueprints, we cap our weekly intensive groups at **12 students**. Contact us or apply online to reserve your mat.
              </p>
